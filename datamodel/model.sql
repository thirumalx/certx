...
--
-- Anchor table -------------------------------------------------------------------------------------------------------
-- US_User table (with 4 attributes)
-----------------------------------------------------------------------------------------------------------------------
...
-- Historized attribute table -----------------------------------------------------------------------------------------
-- US_MNO_User_MobileNumber table (on US_User)
-----------------------------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS certx.US_MNO_User_MobileNumber (
    US_MNO_US_ID bigint not null,
    US_MNO_User_MobileNumber varchar(15) not null,
    US_MNO_ChangedAt timestamp not null,
    Metadata_US_MNO bigint not null,
    constraint fkUS_MNO_User_MobileNumber foreign key (
        US_MNO_US_ID
    ) references certx.US_User (US_ID),
    constraint pkUS_MNO_User_MobileNumber primary key (
        US_MNO_US_ID ,
        US_MNO_ChangedAt
    ) include (
        US_MNO_User_MobileNumber
    )
);
-- Historized attribute table -----------------------------------------------------------------------------------------
-- US_EID_User_EmailId table (on US_User)
-----------------------------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS certx.US_EID_User_EmailId (
    US_EID_US_ID bigint not null,
    US_EID_User_EmailId varchar(150) not null,
    US_EID_ChangedAt timestamp not null,
    Metadata_US_EID bigint not null,
    constraint fkUS_EID_User_EmailId foreign key (
        US_EID_US_ID
    ) references certx.US_User (US_ID),
    constraint pkUS_EID_User_EmailId primary key (
        US_EID_US_ID ,
        US_EID_ChangedAt
    ) include (
        US_EID_User_EmailId
...
-- Anchor table -------------------------------------------------------------------------------------------------------
-- CE_Certificate table (with 8 attributes)
-----------------------------------------------------------------------------------------------------------------------
...
-- Static attribute table ---------------------------------------------------------------------------------------------
-- CE_PAS_Certificate_Password table (on CE_Certificate)
-----------------------------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS certx.CE_PAS_Certificate_Password (
    CE_PAS_CE_ID bigint not null,
    CE_PAS_Certificate_Password varchar(250) not null,
    Metadata_CE_PAS bigint not null,
    constraint fkCE_PAS_Certificate_Password foreign key (
        CE_PAS_CE_ID
    ) references certx.CE_Certificate (CE_ID),
    constraint pkCE_PAS_Certificate_Password primary key (
        CE_PAS_CE_ID 
    ) include (
        CE_PAS_Certificate_Password
    )
);
-- Anchor table -------------------------------------------------------------------------------------------------------
...
-- Historized tie table -----------------------------------------------------------------------------------------------
-- US_worksFor_CL_assignedTo table (having 2 roles)
-----------------------------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS certx.US_worksFor_CL_assignedTo (
    US_ID_worksFor bigint not null, 
    CL_ID_assignedTo bigint not null, 
    US_worksFor_CL_assignedTo_ChangedAt timestamp not null,
    Metadata_US_worksFor_CL_assignedTo bigint not null,
    constraint US_worksFor_CL_assignedTo_fkUS_worksFor foreign key (
        US_ID_worksFor
    ) references certx.US_User(US_ID), 
    constraint US_worksFor_CL_assignedTo_fkCL_assignedTo foreign key (
        CL_ID_assignedTo
    ) references certx.CL_Client(CL_ID), 
    constraint pkUS_worksFor_CL_assignedTo primary key (
        US_ID_worksFor ,
        US_worksFor_CL_assignedTo_ChangedAt 
    )
);
...
;
-- Restatement Finder Function and Constraint -------------------------------------------------------------------------
-- rfUS_MNO_User_MobileNumber restatement finder, also used by the insert and update triggers for idempotent attributes
-- rcUS_MNO_User_MobileNumber restatement constraint (available only in attributes that cannot have restatements)
-----------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION certx.rfUS_MNO_User_MobileNumber(
    id bigint,
    value varchar(15),
    changed timestamp
) RETURNS smallint AS '
    BEGIN
        IF EXISTS (
            ...
            WHERE
                value = (
                    SELECT
                        pre.US_MNO_User_MobileNumber
                    FROM
                        certx.US_MNO_User_MobileNumber pre
                    WHERE
                        pre.US_MNO_US_ID = id
                    AND
                        pre.US_MNO_ChangedAt < changed
                    ORDER BY
                        pre.US_MNO_ChangedAt DESC
                    LIMIT 1
            )
        )
        OR EXISTS(
            ...
            WHERE
                value = (
                    SELECT
                        fol.US_MNO_User_MobileNumber
                    FROM
                        certx.US_MNO_User_MobileNumber fol
                    WHERE
                        fol.US_MNO_US_ID = id
                    AND
                        fol.US_MNO_ChangedAt > changed
                    ORDER BY
                        fol.US_MNO_ChangedAt ASC
                    LIMIT 1
            )
        )
        THEN
...
;
-- Restatement Finder Function and Constraint -------------------------------------------------------------------------
-- rfUS_EID_User_EmailId restatement finder, also used by the insert and update triggers for idempotent attributes
-- rcUS_EID_User_EmailId restatement constraint (available only in attributes that cannot have restatements)
-----------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION certx.rfUS_EID_User_EmailId(
    id bigint,
    value varchar(150),
    changed timestamp
) RETURNS smallint AS '
    BEGIN
        IF EXISTS (
            ...
            WHERE
                value = (
                    SELECT
                        pre.US_EID_User_EmailId
                    FROM
                        certx.US_EID_User_EmailId pre
                    WHERE
                        pre.US_EID_US_ID = id
                    AND
                        pre.US_EID_ChangedAt < changed
                    ORDER BY
                        pre.US_EID_ChangedAt DESC
                    LIMIT 1
            )
        )
        OR EXISTS(
            ...
            WHERE
                value = (
                    SELECT
                        fol.US_EID_User_EmailId
                    FROM
                        certx.US_EID_User_EmailId fol
                    WHERE
                        fol.US_EID_US_ID = id
                    AND
                        fol.US_EID_ChangedAt > changed
                    ORDER BY
                        fol.US_EID_ChangedAt ASC
                    LIMIT 1
            )
        )
        THEN
...
;
-- Latest perspective -------------------------------------------------------------------------------------------------
-- lUS_MNO_User_MobileNumber viewed by the latest available information (may include future versions)
-----------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW certx.lUS_MNO_User_MobileNumber AS
SELECT DISTINCT ON (US_MNO_US_ID) US_MNO_US_ID
     , US_MNO_User_MobileNumber
     , US_MNO_ChangedAt
     , Metadata_US_MNO
  FROM certx.US_MNO_User_MobileNumber
 ORDER 
    BY US_MNO_US_ID DESC
     , US_MNO_ChangedAt DESC 
;
-- Attribute Point-in-time perspective -------------------------------------------------------------------------------------------------
-- pUS_MNO_User_MobileNumber viewed as it was on the given timepoint
-----------------------------------------------------------------------------------------------------------------------
 CREATE OR REPLACE FUNCTION certx.pUS_MNO_User_MobileNumber
      ( changingTimepoint timestamp
      ) 
RETURNS TABLE
      ( US_MNO_US_ID bigint
      , US_MNO_User_MobileNumber varchar(15)
      , US_MNO_ChangedAt timestamp 
      , Metadata_US_MNO bigint
      ) 
AS 
'
 SELECT DISTINCT ON (US_MNO_US_ID) US_MNO_US_ID
      ...
      , Metadata_US_MNO
   FROM certx.US_MNO_User_MobileNumber
  WHERE US_MNO_ChangedAt <= changingTimepoint
  ORDER 
     BY US_MNO_US_ID DESC
      , US_MNO_ChangedAt DESC 
 ;
...
;
-- Now perspective ----------------------------------------------------------------------------------------------------
-- nUS_MNO_User_MobileNumber viewed as it currently is (cannot include future versions)
-----------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW certx.nUS_MNO_User_MobileNumber AS
SELECT *
  FROM certx.pUS_MNO_User_MobileNumber(now()::timestamp)
;
-- Latest perspective -------------------------------------------------------------------------------------------------
-- lUS_EID_User_EmailId viewed by the latest available information (may include future versions)
-----------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW certx.lUS_EID_User_EmailId AS
SELECT DISTINCT ON (US_EID_US_ID) US_EID_US_ID
     , US_EID_User_EmailId
     , US_EID_ChangedAt
     , Metadata_US_EID
  FROM certx.US_EID_User_EmailId
 ORDER 
    BY US_EID_US_ID DESC
     , US_EID_ChangedAt DESC 
;
-- Attribute Point-in-time perspective -------------------------------------------------------------------------------------------------
-- pUS_EID_User_EmailId viewed as it was on the given timepoint
-----------------------------------------------------------------------------------------------------------------------
 CREATE OR REPLACE FUNCTION certx.pUS_EID_User_EmailId
      ( changingTimepoint timestamp
      ) 
RETURNS TABLE
      ( US_EID_US_ID bigint
      , US_EID_User_EmailId varchar(150)
      , US_EID_ChangedAt timestamp 
      , Metadata_US_EID bigint
      ) 
AS 
'
 SELECT DISTINCT ON (US_EID_US_ID) US_EID_US_ID
      ...
      , Metadata_US_EID
   FROM certx.US_EID_User_EmailId
  WHERE US_EID_ChangedAt <= changingTimepoint
  ORDER 
     BY US_EID_US_ID DESC
      , US_EID_ChangedAt DESC 
 ;
...
;
-- Now perspective ----------------------------------------------------------------------------------------------------
-- nUS_EID_User_EmailId viewed as it currently is (cannot include future versions)
-----------------------------------------------------------------------------------------------------------------------
CREATE OR REPLACE VIEW certx.nUS_EID_User_EmailId AS
SELECT *
  FROM certx.pUS_EID_User_EmailId(now()::timestamp)
;
-- Latest perspective -------------------------------------------------------------------------------------------------
...
SELECT US.US_ID
     ...
     , MNO.US_MNO_US_ID
     , MNO.Metadata_US_MNO
     , MNO.US_MNO_ChangedAt
     , MNO.US_MNO_User_MobileNumber
     , EID.US_EID_US_ID
     , EID.Metadata_US_EID
     , EID.US_EID_ChangedAt
     , EID.US_EID_User_EmailId
  ...
  LEFT 
  JOIN certx.lUS_MNO_User_MobileNumber MNO
    ON MNO.US_MNO_US_ID = US.US_ID
  LEFT 
  JOIN certx.lUS_EID_User_EmailId EID
    ON EID.US_EID_US_ID = US.US_ID;
;
...
'
 SELECT US.US_ID
   ...
   JOIN certx.pUS_NAM_User_Name(changingTimepoint) NAM
     ON NAM.US_NAM_US_ID = US.US_ID
   LEFT 
   JOIN certx.pUS_MNO_User_MobileNumber(changingTimepoint) MNO
     ON MNO.US_MNO_US_ID = US.US_ID
   LEFT 
   JOIN certx.pUS_EID_User_EmailId(changingTimepoint) EID
     ON EID.US_EID_US_ID = US.US_ID
;
' 
LANGUAGE SQL STABLE
 ...
 SELECT timepoints.inspectedTimepoint
   ...
   FROM (
          ...
          SELECT DISTINCT US_NAM_US_ID AS US_ID
           ...
           UNION
          SELECT DISTINCT US_MNO_US_ID AS US_ID
               , US_MNO_ChangedAt::timestamp inspectedTimepoint
               , ''MNO'' AS mnemonic
            FROM certx.US_MNO_User_MobileNumber
           WHERE (selection is null OR selection like ''%MNO%'')
             AND US_MNO_ChangedAt BETWEEN intervalStart AND intervalEnd
           UNION
          SELECT DISTINCT US_EID_US_ID AS US_ID
               , US_EID_ChangedAt::timestamp inspectedTimepoint
               , ''EID'' AS mnemonic
            FROM certx.US_EID_User_EmailId
           WHERE (selection is null OR selection like ''%EID%'')
             AND US_EID_ChangedAt BETWEEN intervalStart AND intervalEnd
        ) timepoints
  CROSS 
...
SELECT CE.CE_ID
     ...
     , PAS.CE_PAS_CE_ID
     , PAS.Metadata_CE_PAS
     , PAS.CE_PAS_Certificate_Password
  ...
  LEFT 
  JOIN certx.CE_PAS_Certificate_Password PAS
    ON PAS.CE_PAS_CE_ID = CE.CE_ID;
;
-- Point-in-time perspective ------------------------------------------------------------------------------------------
...
RETURNS TABLE 
      ...
      , CE_STA_STA_ID smallint 
      , CE_PAS_CE_ID bigint
      , Metadata_CE_PAS bigint
      , CE_PAS_Certificate_Password varchar(250)
      ) 
AS 
'
 SELECT CE.CE_ID
...
SELECT current_timestamp
   ...
   "schema": {
      ...
      "tie": {
         ...
         "US_worksFor_CL_assignedTo": {
            "id": "US_worksFor_CL_assignedTo",
            "timeRange": "timestamp",
            "anchorRole": {
               "US_worksFor": {
                  "id": "US_worksFor",
